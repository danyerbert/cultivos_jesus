...
$query=mysqli_query($conexion,"SELECT * FROM cultivos WHERE estatus = '1'");
$documento_cultivos="cultivos.xls";
header('Content-type: application/vnd.ms-excel');
header('Content-Disposition: attachment; filename=' .$documento_cultivos);
header('Pragma: no-cache');
header('Expires: 0');
echo '<table border=1>';
echo '<tr>';
echo '<th colspan=6 rowspan=2>Informacion Cultivos Registrados</th>';
echo '</tr>';
echo '<tr>';
echo '</tr>';
